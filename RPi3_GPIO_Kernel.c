...
static int __init rpi3_gpio_init(void) {
	...
	typedef struct irqNumbers {
		int result1On = 0;
		int result2On = 0;
		int result3On = 0;
	};
	
	struct irqNumbers irqN;
...
/* 	devOn1 = false; // These lines can be used set DEFAULT OFF/ON state for the devices
	...
	irqN.result1On = request_irq(irqNumber1On, // interrupt number requested
	...
	irqN.result2On = request_irq(irqNumber2On,
	...
	irqN.result3On = request_irq(irqNumber3On,
...
static irq_handler_t rpi3_gpio_irq_handler_1_on(unsigned int irq, void *dev_id, struct pt_regs *regs) {
...
static irq_handler_t rpi3_gpio_irq_handler_2_on(unsigned int irq, void *dev_id, struct pt_regs *regs) {
	devOn2 = !devOn2;
	gpio_set_value(gpioDevice2, devOn2);
...
static irq_handler_t rpi3_gpio_irq_handler_3_on(unsigned int irq, void *dev_id, struct pt_regs *regs) {
	devOn3 = !devOn3;
	gpio_set_value(gpioDevice3, devOn3);
